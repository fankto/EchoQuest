...
class JsonMixin:
    ...
    @staticmethod
    def parse_json(value: Union[str, dict, list, None]) -> Optional[Union[dict, list]]:
    ...
    @staticmethod
    def serialize_json(value: Optional[Union[dict, list]]) -> Optional[str]:
...
interview_questionnaire = Table(
    ...
    Base.metadata,
    Column("interview_id", UUID(as_uuid=True), ForeignKey("interviews.id", ondelete="CASCADE"), primary_key=True),
    Column("questionnaire_id", UUID(as_uuid=True), ForeignKey("questionnaires.id", ondelete="CASCADE"),
           primary_key=True),
...
class User(Base):
    ...
    questionnaires = relationship("Questionnaire", back_populates="creator", cascade="all, delete-orphan")
...
class Organization(Base):
    ...
    questionnaires = relationship("Questionnaire", back_populates="organization", cascade="all, delete-orphan")
...
class Questionnaire(Base):
    ...
    questions = Column(JSON, nullable=False, default=list)  # List of questions
    ...
    def validate_questions(self, key, value):
        """Validate questions is always a list"""
        if value is None:
            return []
        return value


class Interview(Base, JsonMixin):
    """Interview model"""
    ...
    # Generated answers - map questionnaire_id to answers
    generated_answers = Column(JSON, nullable=True)  # Dictionary mapping questionnaire_id to answers
    ...
    # Relationships - Many-to-many with questionnaires
...
class Transaction(Base):
    ...
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    organization_id = Column(UUID(as_uuid=True), ForeignKey("organizations.id", ondelete="SET NULL"), nullable=True)
    interview_id = Column(UUID(as_uuid=True), ForeignKey("interviews.id", ondelete="SET NULL"), nullable=True)
