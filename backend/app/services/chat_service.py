...
from app.core.exceptions import InsufficientCreditsError, ExternalAPIError
...
class ChatService:
    ...
    def __init__(self):
        ...
        self.valid_api_key = self._check_api_key()
        logger.info(f"Chat service initialized with OpenAI API key valid: {self.valid_api_key}")
    ...
    async def generate_assistant_response(
        ...
        try:
            ...
            if redis:
                cache_key = f"chat:response:{interview.id}:{hash(str(messages))}"
                cached_response = await redis.get(cache_key)
                if cached_response:
                    content = cached_response.decode('utf-8')
                    logger.info(f"Using cached response for interview {interview.id}")
                else:
                    # Call OpenAI API
    ...
    async def _get_transcript_context(
        ...
        redis = await get_redis_client()
        if redis:
            cache_key = f"transcript:context:{interview.id}:{hash(last_user_message.content)}"
...
{transcript_context}"""
    ...
    async def stream_assistant_response(
        ...
        try:
            ...
            yield f"data: {json.dumps({'type': 'error', 'content': str(e), 'code': 'insufficient_credits'})}\n\n"
            await db.rollback()
            raise
        except Exception as e:
